...
Please provide these details so I can give you a comprehensive portfolio recommendation with real stock prices and proper allocation!`;
  ...
  const handleStockQuestionnaireComplete = async (preferences: StockAnalysisPreferences, messageId: string) => {
    try {
      ...
      // Check if OpenAI provided a comprehensive analysis with reasoning
      if (stockAnalysis.recommendation?.reasoning && Array.isArray(stockAnalysis.recommendation.reasoning) && 
          stockAnalysis.recommendation.reasoning.length > 0) {
